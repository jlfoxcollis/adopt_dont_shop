...
RSpec.describe 'admin shelters index' do
  ...
  it "can can show shelters in rev order" do
    ...
    expect(page.all('li')[6]).to have_content("#{@shelter2.name}")
    expect(page.all('li')[7]).to have_content("#{@shelter3.name}")
    expect(page.all('li')[8]).to have_content("#{@shelter1.name}")
    expect(page.all('li')[9]).to have_content("#{@shelter1.name}")
    expect(page.all('li')[10]).to have_content("#{@shelter2.name}")
  end
